...
def create_todo_list_route(
    todo_list: TodoListCreate, db: Session = Depends(get_db)
) -> TodoListRead:
...
def get_all_todo_lists_route(db: Session = Depends(get_db)):
    repository = TodoListRepositoryImpl(db)
    todos = get_all_todo_lists(repository=repository)
...
def get_todo_list_route(todo_list_id: int, db: Session = Depends(get_db)):
    repository = TodoListRepositoryImpl(db)
    todo = get_todo_list_by_id(repository=repository, todo_list_id=todo_list_id)
    if not todo:
...
def update_todo_list_route(
    todo_list_id: int, todo_list: TodoListCreate, db: Session = Depends(get_db)
):
    repository = TodoListRepositoryImpl(db)
...
def delete_todo_list_route(todo_list_id: int, db: Session = Depends(get_db)):
    repository = TodoListRepositoryImpl(db)
    success = delete_todo_list(repository=repository, todo_list_id=todo_list_id)
    if not success:
        raise HTTPException(status_code=404, detail="Todo list not found")
